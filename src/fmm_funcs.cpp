...
#include "./fmm_interactions/inverse_bilaplacian_interactions.hpp"
#include "./fmm_interactions/bve_interactions.hpp"
#include "./fmm_interactions/sal_interactions.hpp"
...
void fmm_inverse_bilaplacian(const RunConfig& run_information, const std::vector<InteractPair>& interactions, const std::vector<CubePanel>& source_tree,
						   const std::vector<CubePanel>& target_tree, const std::vector<double>& xcos, const std::vector<double>& ycos, 
						   const std::vector<double>& zcos, const std::vector<double>& area, const std::vector<double>& potential, 
						   const std::vector<int>& point_source_leaf, std::vector<double>& integral) {
	// use an upward pass and downward pass to compute proxy source weights/proxy target potentials
	...
	for (int i = 0; i < interactions.size(); i++) {
		if (i % run_information.mpi_P == run_information.mpi_ID) {
			...
			i_s = interactions[i].index_source;
			if (interactions[i].interact_type == 0) { // PP
				fmm_pp_interaction_inverse_bilaplacian(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, potential, integral);
			} else if (interactions[i].interact_type == 1) { // PC
				fmm_pc_interaction_inverse_bilaplacian(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, proxy_source_weights[i_s], integral);
			} else if (interactions[i].interact_type == 2) { // CP
				fmm_cp_interaction_inverse_bilaplacian(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, potential, proxy_target_potenti[i_t]);
			} else { // CC
				fmm_cc_interaction_inverse_bilaplacian(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, proxy_source_weights[i_s], proxy_target_potenti[i_t]);
			}
		}
	}
...
void fmm_sal(const RunConfig& run_information, const std::vector<InteractPair>& interactions, const std::vector<CubePanel>& source_tree,
						   const std::vector<CubePanel>& target_tree, const std::vector<double>& xcos, const std::vector<double>& ycos, 
						   const std::vector<double>& zcos, const std::vector<double>& area, const std::vector<double>& potential, 
						   const std::vector<int>& point_source_leaf, std::vector<double>& integral) {
	// use an upward pass and downward pass to compute proxy source weights/proxy target potentials
	...
	for (int i = 0; i < interactions.size(); i++) {
		if (i % run_information.mpi_P == run_information.mpi_ID) {
			...
			i_s = interactions[i].index_source;
			if (interactions[i].interact_type == 0) { // PP
				fmm_pp_interaction_sal(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, potential, integral);
			} else if (interactions[i].interact_type == 1) { // PC
				fmm_pc_interaction_sal(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, proxy_source_weights[i_s], integral);
			} else if (interactions[i].interact_type == 2) { // CP
				fmm_cp_interaction_sal(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, potential, proxy_target_potenti[i_t]);
			} else { // CC
				fmm_cc_interaction_sal(run_information, source_tree[i_s], target_tree[i_t], xcos, ycos, zcos, area, proxy_source_weights[i_s], proxy_target_potenti[i_t]);
			}
		}
	}
