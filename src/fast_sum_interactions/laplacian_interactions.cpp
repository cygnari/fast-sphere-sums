...
void pp_interaction_laplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
  ...
  for (int i = 0; i < cube_panel_target.point_count; i++) {
    ...
    for (int j = 0; j < source_count; j++) {
      if (target_i != cube_panel_source.points_inside[j]) {
        ...
        integral[target_i] += (h1+h2)/pow(eps,2.5) * (pots[target_i]-pots[j]) * areas[j];
...
void pc_interaction_laplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
                    const std::vector<double>& xcos, const std::vector<double>& ycos, const std::vector<double>& zcos,
                    const std::vector<double>& area, const std::vector<double>& potential, std::vector<double>& integral) {
  // pc interaction
  ...
  for (int i = 0; i < count_target; i++) {
    ...
    for (int j = 0; j < degree+1; j++) { // xi loop
      ...
      for (int k = 0; k < degree+1; k++) { // eta loop
        ...
        gamm = tx * xyz[0] + ty * xyz[1] + tz * xyz[2];
        gamm2 = gamm * gamm;
        gamm4 = gamm2 * gamm2;
        h1 = exp(-1/eps)*pow(M_PI,-1.5)*(2*gamm*(-1.0+3*eps+gamm2));
        h2 = exp((gamm2-1)/eps)/(sqrt(eps)*M_PI)*(2*eps*eps+2*gamm2*(gamm2-1)+eps*(7*gamm2-1))*(1+std::erf(gamm/eps));
        integral[point_index] += (h1+h2)/pow(eps,2.5) * (potential[point_index]-proxy_weights[j][k]);
...
void cp_interaction_laplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
                    const std::vector<double>& xcos, const std::vector<double>& ycos, const std::vector<double>& zcos,
                    const std::vector<double>& area, const std::vector<double>& potential, std::vector<double>& integral) {
  // cp interaction
...
void cc_interaction_laplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
                    const std::vector<double>& xcos, const std::vector<double>& ycos, const std::vector<double>& zcos,
                    const std::vector<double>& area, const std::vector<double>& potential, std::vector<double>& integral) {
  // cc interaction
  int point_index, count_target = cube_panel_target.point_count, count_source = cube_panel_source.point_count, degree=run_information.interp_degree;
