...
void fmm_pp_interaction_inverse_bilaplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
...
void fmm_pc_interaction_inverse_bilaplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
                    const std::vector<double>& xcos, const std::vector<double>& ycos, const std::vector<double>& zcos,
  ...
  for (int i = 0; i < count_target; i++) {
    ...
    for (int j = 0; j < degree+1; j++) { // xi loop
      ...
      for (int k = 0; k < degree+1; k++) { // eta loop
        ...
        integral[point_index] += 1.0/(4.0*M_PI)*dilog(0.5*(1+tx*xyz[0]+ty*xyz[1]+tz*xyz[2])) * proxy_source_weights[j][k];
      }
    }
  }
}

void fmm_cp_interaction_inverse_bilaplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
                    const std::vector<double>& xcos, const std::vector<double>& ycos, const std::vector<double>& zcos,
  ...
  for (int i = 0; i < degree+1; i++) { // xi loop
    for (int j = 0; j < degree+1; j++) { // eta loop
      ...
      for (int k = 0; k < count_source; k++) {
        ...
        proxy_target_potenti[i][j] += 1.0/(4.0*M_PI)*dilog(0.5*(1+xyz[0]*sx+xyz[1]*sy+xyz[2]*sz)) * areas[k]*pots[k];
      }
    }
  }
}

void fmm_cc_interaction_inverse_bilaplacian(const RunConfig& run_information, const CubePanel& cube_panel_source, const CubePanel& cube_panel_target,
                    const std::vector<double>& xcos, const std::vector<double>& ycos, const std::vector<double>& zcos,
                    const std::vector<double>& area, const std::vector<std::vector<double>>& proxy_source_weights, 
                    std::vector<std::vector<double>>& proxy_target_potenti) {
  // cc interaction
    ...
    for (int j = 0; j < degree+1; j++) { // target eta
      ...
      for (int k = 0; k < degree+1; k++) { // source xi
        ...
        for (int l = 0; l < degree+1; l++) { // source eta
          ...
          proxy_target_potenti[i][j] += 1.0/(4.0*M_PI)*dilog(0.5*(1+cxs*cxt+cys*cyt+czs*czt))*proxy_source_weights[k][l];
        }
      }
    }
  }
