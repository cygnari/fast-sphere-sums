...
int main(int argc, char **argv) {
  ...
  MPI_Win win_integrated_1, win_integrated_2, win_integrated_3;
  ...
  MPI_Win_create(&integrated_1[0], run_information.point_count * sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_integrated_1);
  MPI_Win_create(&integrated_2[0], run_information.point_count * sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_integrated_2);
  MPI_Win_create(&integrated_3[0], run_information.point_count * sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_integrated_3);
  ...
  if (run_information.use_fast) {
    ...
    begin = std::chrono::steady_clock::now();
    direct_sum_bve(run_information, xcos, ycos, zcos, area, potential, integrated_1, integrated_2, integrated_3);
    sync_updates<double>(integrated_1, P, ID, &win_integrated_1, MPI_DOUBLE);
    sync_updates<double>(integrated_2, P, ID, &win_integrated_2, MPI_DOUBLE);
    sync_updates<double>(integrated_3, P, ID, &win_integrated_3, MPI_DOUBLE);
    end = std::chrono::steady_clock::now();
  ...
  if (ID == 0) {
    ...
    if (run_information.write_output) {
      std::string output_folder = create_config(run_information) + "_bve";
