...
int main(int argc, char ** argv) {
  ...
  std::vector<double> dyn_xco (tri_count, 0);
  std::vector<double> dyn_yco (tri_count, 0);
  std::vector<double> dyn_zco (tri_count, 0);
  std::vector<double> dyn_areas (tri_count, 0);
  ...
  for (int i = 0; i < tri_count; i++) {
    ...
    dyn_areas[i] = tri_area;
    v123[0] = (v1[0] + v2[0] + v3[0])/3.0;
    v123[1] = (v1[1] + v2[1] + v3[1])/3.0;
    v123[2] = (v1[2] + v2[2] + v3[2])/3.0;
    project_to_sphere(v123, 1.0);
    dyn_xco[i] = v123[0];
    dyn_yco[i] = v123[1];
    dyn_zco[i] = v123[2];
  }

  std::string prefix = DATA_DIR + std::to_string(tri_count) + "_icos_grid_";
  write_state(dyn_xco, prefix, "x.csv");
  write_state(dyn_yco, prefix, "y.csv");
  write_state(dyn_zco, prefix, "z.csv");
  write_state(dyn_areas, prefix, "areas.csv");
