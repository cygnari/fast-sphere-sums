...
int main(int argc, char **argv) {
  ...
  if (run_information.use_fast) {
    ...
    begin = std::chrono::steady_clock::now();
    direct_sum_invert_laplacian_reg(run_information, xcos, ycos, zcos, area, potential, integrated);
    sync_updates<double>(integrated, P, ID, &win_integrated, MPI_DOUBLE);
    end = std::chrono::steady_clock::now();
  }
  ...
  if (ID == 0) {
    ...
    if (run_information.write_output) {
      std::string output_folder = create_config(run_information) + "_inv_lap_reg";
